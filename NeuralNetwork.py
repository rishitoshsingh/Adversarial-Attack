...
class ANN:
    ...
    def get_adversial_image(self, X_target, y_goal, learning_rate=0.01, _lambda=0.05, epochs=200, print_results=False):
        ...
        for epoch in range(epochs):
            self._forward_pass(x) # forward pass (update node["output"])
            ...
            self.train_loss.append(0.5 * self.train_epoch_error + _lambda * np.sum(np.square(X_target-x)))
            if print_results: #and epoch%100 == 0:
                print("Epoch --- ",epoch," MSE : ",self.train_loss[-1])
            self.train_epoch_error.clear()
        return x
    ...
    def _update_inputs_adversial(self, x, X_target, learning_rate, _lambda):
        for i, _input in enumerate(x):
            _input += learning_rate * self.network[0][i]['delta'] + 2 * _lambda * (X_target[i] - _input) 
        return x